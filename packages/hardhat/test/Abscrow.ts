...
describe('Abscrow', function () {
  let owner : SignerWithAddress, buyer: SignerWithAddress, nonBuyer: SignerWithAddress, seller: SignerWithAddress;
  let eCedi: ECedi, abscrowContract: Abscrow;
  const totalAmount = ethers.utils.parseUnits('1', 6); // 100 tokens
  const deadline = 1000000000

  before(async () => {
    [owner, buyer, nonBuyer, seller] = await ethers.getSigners();
  ...
  it('should allow the buyer to release funds', async () => {
    const purchaseId = 1;
    await abscrowContract.connect(buyer).release(purchaseId, { gasLimit: 3e7 });
